...
func newTodo(db *gorm.DB, opts ...gen.DOOption) todo {
	...
	_todo.todoDo.UseModel(&data.Todo{})
	...
	_todo.State = field.NewInt(tableName, "state")
...
type ITodoDo interface {
	...
	Unscoped() ITodoDo
	Create(values ...*data.Todo) error
	CreateInBatches(values []*data.Todo, batchSize int) error
	Save(values ...*data.Todo) error
	First() (*data.Todo, error)
	Take() (*data.Todo, error)
	Last() (*data.Todo, error)
	Find() ([]*data.Todo, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*data.Todo, err error)
	FindInBatches(result *[]*data.Todo, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*data.Todo) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*data.Todo, error)
	FirstOrCreate() (*data.Todo, error)
	FindByPage(offset int, limit int) (result []*data.Todo, count int64, err error)
...
func (t todoDo) Create(values ...*data.Todo) error {
	if len(values) == 0 {
		return nil
	}
...
func (t todoDo) CreateInBatches(values []*data.Todo, batchSize int) error {
	return t.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (t todoDo) Save(values ...*data.Todo) error {
	if len(values) == 0 {
		return nil
	}
...
func (t todoDo) First() (*data.Todo, error) {
	if result, err := t.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*data.Todo), nil
	}
}

func (t todoDo) Take() (*data.Todo, error) {
...
func (t todoDo) Last() (*data.Todo, error) {
...
func (t todoDo) Find() ([]*data.Todo, error) {
	result, err := t.DO.Find()
	return result.([]*data.Todo), err
}

func (t todoDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*data.Todo, err error) {
	buf := make([]*data.Todo, 0, batchSize)
...
func (t todoDo) FindInBatches(result *[]*data.Todo, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (t todoDo) FirstOrInit() (*data.Todo, error) {
...
func (t todoDo) FirstOrCreate() (*data.Todo, error) {
...
func (t todoDo) FindByPage(offset int, limit int) (result []*data.Todo, count int64, err error) {
...
func (t todoDo) Delete(models ...*data.Todo) (result gen.ResultInfo, err error) {
